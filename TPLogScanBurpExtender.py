...
class BurpExtender(IBurpExtender, IContextMenuFactory, ITab, IHttpListener, IMessageEditorController, AbstractTableModel):
    ...
    def logScan(self, message, version, log_path, filename_list):
        for filename in filename_list:
            try:
                response, status_code = self.sendRequest(message, log_path+filename)
                if status_code != 200:
                    continue
                ...
                while True:
                    ...
                    response, status_code = self.sendRequest(message, log_path+tmp_filename)
                    if status_code != 200:
                        break
    ...
    def sendRequest(self, message, path):
        old_request = self._helpers.bytesToString(message.getRequest())
        old_path = self._helpers.analyzeRequest(message).getUrl().getPath()
        new_request = old_request.replace(" " + old_path + " HTTP/", " " + path + " HTTP/")
        checkRequestResponse = self._callbacks.makeHttpRequest(message.getHttpService(), self._helpers.stringToBytes(new_request))
...
class TPLogScan():
    ...
    def genFileName(self):
        ...
        month_list = []
        if year < now_year:
            month_list = [(year, i) for i in range(month, 13)] + [(now_year, i) for i in range(1, now_month+1)]
        else:
            month_list = [(year, i) for i in range(month,now_month+1)]
        filename_list = []
        for y,i in month_list:
            start = 1
            end = 32
            if i == now_month:
                end = now_day+1
            ...
            for j in range(start,end):
                # if i == now_month and j > now_day:
                #     break
                if version == 3:
                    filename_list.append("{:0>2d}_{:0>2d}_{:0>2d}.log".format(int(str(y)[2:]), i, j))
                elif version == 5:
                    filename_list.append("{}{:0>2d}/{:0>2d}.log".format(y, i, j))
